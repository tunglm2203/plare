    ...
    **kwargs,
    ...
    for y_key in y_keys:
        ...
        if all(len(xs[0]) == len(x) for x in xs):
            ...
            print("\033[94m Max: {:.1f} \\stdv{{{:.1f}}} [{} seeds] \033[0m".format(max_mean * 100, max_std * 100, ys.shape[0]), label)
            # print("\033[94m {:.1f} \\stdv{{{:.1f}}} \033[0m".format(means[-1] * 100, stds[-1] * 100))
            # print("\033[94m Last: {:.1f} \\stdv{{{:.1f}}} [{} seeds] \033[0m".format(means[-1] * 100, stds[-1] * 100, ys.shape[0]), label)
            print()
            xs = xs.flatten()
            ys = ys.flatten()
        else:
...
def create_plot(
    ...
    max_seeds: Optional[int] = 100,
    **kwargs,
):
    assert len(paths) == len(labels), "The length of paths must the same as the length of labels"
