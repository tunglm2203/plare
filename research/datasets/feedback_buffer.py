...
class FeedbackBuffer(torch.utils.data.IterableDataset):
    def __init__(
        ...
        ignore_unsure: Optional[bool] = False,
    ):
        ...
        self.ignore_unsure = ignore_unsure
        ...
        with open(path, "rb") as f:
            ...
            if self.label_key in ['partial_return']:
                utils.print_yellow(f"Using different teacher: {self.label_key}")
            else:
    ...
    def __iter__(self):
        ...
        for i in range(math.ceil(len(idxs) / self.batch_size)):  # Need to use ceil to get all data points.
            ...
            if self.mode == "score":
                ...
                if self.label_key == 'vlm_label':
                    hard_label = 1.0 * (self.data[self.label_key][data_1_idxs] == 1)
                    soft_label = 0.5 * (self.data[self.label_key][data_1_idxs] == -1)
                elif self.label_key == 'partial_return':
                    sum_reward_1 = np.sum(batch['reward_1'], axis=1)
                    sum_reward_2 = np.sum(batch['reward_2'], axis=1)
