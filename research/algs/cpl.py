...
def biased_bce_with_logits(adv1, adv2, y, bias=1.0, ignore_unsure=False):
...
class CPL(OffPolicyAlgorithm):
    ...
    def _get_cpl_loss(self, batch):
        ...
        if "score" in batch:
            ...
            cpl_loss, accuracy = biased_bce_with_logits(adv1, adv2, batch["label"].float(), bias=self.contrastive_bias, ignore_unsure=self.dataset.ignore_unsure)
