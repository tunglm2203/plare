...
if __name__ == "__main__":
    ...
    parser.add_argument("--x", "-x", type=str, default="time/step", help="X value to plot")
    ...
    parser.add_argument("--y", "-y", type=str, nargs="+", default=["eval/success"], help="Y value(s) to plot")
    ...
    if len(paths) == 1 and paths[0].endswith(".yaml"):
        ...
        exp_names = []
        for path in paths:
            exp_names.append(os.path.basename(path))
        color_map = {}
        for i in range(len(exp_names)):
            color_map[exp_names[i]] = i
        ...
        paths, labels = zip(*zip(paths, labels))
